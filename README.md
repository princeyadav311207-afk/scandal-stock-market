...
For each revealed statement, players bet `₹20` to `₹100` on the person they think chose it. Bets are deducted when placed. Losing bets form the losing pool. All winning bettors split that losing pool equally.

Winning bets return:

```text
original wager + equal share of the losing pool
```

Example: if Ridhima bets `₹60` correctly and the losing pool is `₹140`, her returned payout is `₹200`. Her net balance gain for that round is `₹140`, because her original `₹60` stake comes back too.
...
The tests cover one correct person, multiple correct people, nobody correct, one-to-three player statement choices, self-bets, normal bets, multiple winners, unequal losing amounts, wager-plus-winnings returns, no-winner policy, and insufficient balance.
