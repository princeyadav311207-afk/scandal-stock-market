(function (root) {
  ...
  function validateBet(bet, playersById, options = {}) {
    ...
    const availableBalance = options.betsAlreadyDeducted ? bettor.balance + bet.amount : bettor.balance;
    if (availableBalance < bet.amount) throw new Error("Insufficient balance");
  }
  ...
  function resolveRound(statementId, bets, players, options = {}) {
    ...
    bets.forEach((bet) => validateBet(bet, playersById, { betsAlreadyDeducted }));
    ...
    const updatedPlayers = players.map((player) => {
      ...
      playerBets.forEach((bet) => {
        ...
        if (bet.won) {
          const grossReturn = money(bet.amount + payoutPerWinner);
          delta += grossReturn;
          bet.payout = grossReturn;
          bet.winnings = payoutPerWinner;
          bet.balanceDelta = payoutPerWinner;
