...
test("normal correct and incorrect bets resolve against actual statement choosers", () => {
  ...
  assert.equal(result.bets[0].payout, 100);
  assert.equal(result.players.find((p) => p.id === "aman").balance, 1040);
  assert.equal(result.players.find((p) => p.id === "rohan").balance, 960);
});

test("winner gets wager back plus winner share as returned payout", () => {
  const result = E.resolveRound("7", [
    { id: "b1", bettorId: "aman", targetId: "rohan", amount: 80 },
    { id: "b2", bettorId: "priya", targetId: "rahul", amount: 60 },
    { id: "b3", bettorId: "rohan", targetId: "aman", amount: 20 },
    { id: "b4", bettorId: "rahul", targetId: "aman", amount: 40 },
  ], players({ priya: { chosenStatementId: "5" } }));

  const winningBet = result.bets.find((bet) => bet.bettorId === "priya");
  assert.equal(result.losingPool, 140);
  assert.equal(result.payoutPerWinner, 140);
  assert.equal(winningBet.payout, 200);
  assert.equal(result.players.find((p) => p.id === "priya").balance, 1140);
});
...
test("self-bet wins when exactly one player chose the statement", () => {
  ...
  assert.equal(result.bets[0].payout, 180);
  assert.equal(result.players.find((p) => p.id === "rahul").balance, 1080);
});
...
test("self-bet loses when multiple players chose the statement", () => {
  ...
  assert.equal(result.bets[1].payout, 160);
  assert.equal(result.players.find((p) => p.id === "aman").balance, 1100);
});
...
test("multiple winners split unequal losing amounts equally", () => {
  ...
  assert.equal(result.bets[0].payout, 110);
  assert.equal(result.bets[1].payout, 130);
  assert.equal(result.players.find((p) => p.id === "aman").balance, 1090);
  assert.equal(result.players.find((p) => p.id === "rohan").balance, 1090);
});
...
test("already deducted bets validate against balance plus current stake", () => {
  const result = E.resolveRound("7", [
    { id: "b1", bettorId: "rahul", targetId: "rahul", amount: 80 },
    { id: "b2", bettorId: "aman", targetId: "rohan", amount: 20 },
  ], players({
    rahul: { chosenStatementIds: ["7"], balance: 0 },
    priya: { chosenStatementId: "5" },
    aman: { balance: 980 },
  }), { betsAlreadyDeducted: true });

  assert.equal(result.bets[0].payout, 100);
  assert.equal(result.players.find((p) => p.id === "rahul").balance, 100);
});
